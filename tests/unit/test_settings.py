...
def test_default_paths(mock_settings_no_credentials):
    """Test that default paths are set correctly."""
    settings = mock_settings_no_credentials

    assert settings.paths.data_dir is not None
    assert settings.paths.token_file is not None
    assert settings.paths.activities_cache_file is not None
    assert settings.paths.streams_dir is not None


def test_sync_config_defaults(mock_settings):
    """Test that sync configuration has sensible defaults."""
    settings = mock_settings

    assert settings.sync.max_pages > 0
    assert settings.sync.retry_interval_seconds > 0
    assert isinstance(settings.sync.skip_trainer_activities, bool)
